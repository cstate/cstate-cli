...
function normalizeRecordSeverity(value) {
	return ['none', 'notice'].includes(value) ? value : 'none';
}
...
recordKind: ${recordType.recordKind || recordType.recordType}`;
	...
	if (recordType.recordType === 'experiment' || answers.severity) {
		frontmatter += `\nseverity: ${normalizeRecordSeverity(answers.severity || recordType.severityDefault)}`;
	}

	if (recordType.recordType === 'experiment' || answers.pin !== undefined) {
		frontmatter += `\npin: ${Boolean(answers.pin ?? recordType.pinDefault)}`;
	}

	frontmatter += formatYamlList(answers.affected);

	if (answers.summary) {
		frontmatter += `\nsummary: ${formatYamlString(answers.summary)}`;
	}
